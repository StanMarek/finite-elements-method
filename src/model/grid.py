...
class Grid:
	...
	def display(self):
		...
		dX: {:.3f}, dY: {:.3f}
		"""
		print(s.format(self.H, self.B, self.nN, self.nE, self.delta_x, self.delta_y))
		...
		for _ in range(columns):
			for r in range(rows):
				print(('\033[91m'+"{}".format(self.elements[el_id + r].ID[0]) + '\033[0m' + "- " * unit), end='')
			print('\033[91m' + "{}".format(self.elements[last_elem_in_column].ID[3]) + '\033[0m')
	
			for j in range(unit):
				if j == (int)(unit/2):
					for c in range(rows_start, columns + 1 + rows_start, 1):
						print("|" + "  " * int(unit / 2) + "{}".format(c + 1) + "  " * int(unit / 2), end ="")
					rows_start += rows
				print(("|" + "  " * unit) * rows + "|")
		...
		for r in range(rows):
			...
			print(('\033[91m' + "{}".format(self.elements[last_column].ID[1]) + '\033[0m' + "- " * unit), end='')
		print('\033[91m' + "{}".format(self.elements[self.nE - 1].ID[2]) + '\033[0m')
